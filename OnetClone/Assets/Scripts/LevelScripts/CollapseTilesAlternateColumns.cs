...
public class CollapseTilesAlternateColumns : ICollapseStrategy
{
    ...
    public CollapseTilesAlternateColumns(MonoBehaviour monoBehaviour, Tile[,] tiles, int gridWidth, int gridHeight)
    {
                            ...
                            for (int i = y; i > 0; i--)
                            {
                                if (!tiles[x, i - 1].isEmpty)
                                {
                                    ...
                                    activeMovements.Add(monoBehaviour.StartCoroutine(
                                        MoveTileToPosition(tiles[x, i], new Vector3(x, i, 0))
                                    ));
                                    activeMovements.Add(monoBehaviour.StartCoroutine(
                                        MoveTileToPosition(tiles[x, i - 1], new Vector3(x, i - 1, 0))
                                    ));
